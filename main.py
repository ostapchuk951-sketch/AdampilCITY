...
import json
import logging
import random  # Додано імпорт модуля random
...
async def send_reminder(application: Application):
    ...
    # Список варіантів нагадувань
    reminder_messages = [
        "Не забудь зробити ковток води. 💧",
        "Час зволожити організм — випий води. 💦",
        "Пора освіжитися склянкою водички. 💧",
        "Підтримай себе — зроби кілька ковтків води. 💦",
        "Додай енергії — випий трохи води. 💧",
        "Нагадую: тілу потрібна вода, зроби ковток. 💦",
        "Вода — це життя! Пора поповнити запаси. 💧",
        "Твій організм потребує зволоження. Випий води! 💦",
        "Час для водної паузи! Зроби ковток. 💧",
        "Залишайся гідратованим протягом дня. Випий води! 💦"
    ]
    ...
    for user_id, data in users.items():
        if data.get("reminder"):
            ...
            try:
                # Випадково обираємо повідомлення зі списку
                message = random.choice(reminder_messages)
                await application.bot.send_message(
                    chat_id=chat_id,
                    text=f"{message} Твоя денна норма: {water_amount} л."
                )
